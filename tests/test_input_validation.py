...
# These tests assert schema-level rejection, which happens before the
# service validates the model name — but the value still has to be one the
# API accepts, so that a future failure here means what it says.
MODEL = GROQ_MODEL


def test_create_tool_empty_name_rejected(client):
    response = client.post(
        "/tools", json={"name": "", "description": "Valid description"}
    )
...
def test_create_tool_empty_description_rejected(client):
    response = client.post(
        "/tools", json={"name": "valid-name", "description": ""}
    )
    assert response.status_code == 422
...
def test_create_tool_name_too_long_rejected(client):
    response = client.post(
        "/tools", json={"name": "x" * 201, "description": "Valid description"}
    )
    assert response.status_code == 422
...
def test_update_tool_none_name_allowed(client):
    ...
    response = client.put(
        f"/tools/{tool['id']}", json={"description": "Updated description"}
    )
...
def test_run_agent_empty_task_rejected(client):
    ...
    response = client.post(
        f"/agents/{agent['id']}/run", json={"task": "", "model": MODEL}
    )
    assert response.status_code == 422
...
def test_run_agent_invalid_model_returns_400(client):
    ...
    response = client.post(
        f"/agents/{agent['id']}/run",
        json={"task": "valid task", "model": "invalid-model"},
    )
...
def test_run_agent_missing_model_returns_422(client):
    ...
    response = client.post(
        f"/agents/{agent['id']}/run", json={"task": "valid task"}
    )
    assert response.status_code == 422
